...
class KineticNeuralNetworkBuilder(ModelBuilder):
    ...
    def _build_rates(self, inputs_op):
        ...
        for i, rate in enumerate(self.kinn.rates):
            ...
            seq_range_d = (
                min(self.n_feats - 1, rate.input_range[1]) - rate.input_range[0]
            )

            if self.replace_conv_by_fc:  # if replace is True, overwrite the rate dict
                padding = "valid"
            else:
                padding = rate.__dict__.get(
            ...
            conv_rate = Conv1D(
                filters=filters,
                kernel_size=(seq_range_d,)
                if self.replace_conv_by_fc
                else self.rate_pwm_len[i],
                activation="linear",
                # use_bias=False,
                kernel_initializer=self.weight_initializer()
                if callable(self.weight_initializer)
            ...
            if hidden_size == 0:
                ...
                reshape_map = {
                    0: tf.keras.layers.Flatten,
                    1: tf.keras.layers.GlobalAveragePooling1D,
                    2: tf.keras.layers.GlobalMaxPooling1D,
                }
        ...
        king_altman = Lambda(
            lambda x: tf.nn.softmax(
                tf.matmul(
                    x,
                    tf.constant(
                        self.kinn.get_ka_pattern_mat().transpose(), dtype=tf.float32
                    ),
                )
            ),
            name="KingAltman",
    ...
    def _build_activity(self, rates, king_altman):
        ...
        for k in range(rate_contrib_mat.shape[1]):
            ...
            rate_layer = Lambda(
                lambda x: tf.math.exp(x), name=f"exp_k{rate_index}_{k}"
            )(rate_layer)
            ka_slice = Lambda(
                lambda x: tf.gather(x, [k], axis=-1), name=f"KA_slice_{k}"
            )(king_altman)
            intermediate = Concatenate(name=f"gather_act_{k}")([ka_slice, rate_layer])
            activity.append(
                Lambda(
    ...
    def build(
        self, optimizer=None, output_act=False, plot=False, return_intermediate=False
    ):
        ...
        if output_act is True:
            self.model = Model(
                [inputs_op[j] for j in inputs_op],
    ...
    def load_data(self, fp, output_act=False):
        ...
        data["seq_ohe"] = [
            enc.transform([[t] for t in row[1][0:50]]) for row in data.iterrows()
        ]
        if output_act is True:
            data["obs"] = data["50"]
...
class KineticMatrixLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        num_states: int,
        scatter_ind: list,
        regularize_eigenval_ratio: float,
        top_k_eigvals=3,
    ):
        super().__init__()
    ...
    def _scatter_single_sample(self, input_tensor):
        ...
        for i in range(self.num_rates):
            ...
            for ind, sign in self.scatter_ind[i]:
                ...
                inds.append(ind)
        kinetic_matrix = tf2.scatter_nd(
            inds, updates, (self.num_states, self.num_states)
        )
    ...
    def call(self, inputs: tf.Tensor):
        ...
        if self.regularize_eigenval_ratio > 0:
            ...
            self.add_loss(
                self.regularize_eigenval_ratio
                * tf.reduce_mean(
                    tf.math.divide_no_nan(
                        batch_top_ev.values[:, 1], batch_top_ev.values[:, 2]
                    )
                )
            )
...
class KineticEigenModelBuilder(KineticNeuralNetworkBuilder):
    def __init__(
        ...
        self.weight_initializer = lambda: tf.keras.initializers.RandomNormal(
            stddev=0.01
        )
        self.regularize_eigenval_ratio = regularize_eigenval_ratio
    ...
    def _build_activity(self, rates):
        ...
        kinetic_eig = KineticMatrixLayer(
            num_rates=len(rates),
            num_states=len(self.kinn.states),
            scatter_ind=scatter_ind,
            regularize_eigenval_ratio=self.regularize_eigenval_ratio,
        )(concat)
        return [kinetic_eig]
    ...
    def build(self, optimizer=None, output_act=False, plot=False):
        ...
        optimizer = optimizer or tf2.keras.optimizers.SGD(
            lr=0.1, momentum=0.95, decay=1e-5
        )
        if output_act is True:
            self.model = tf2.keras.models.Model(
                [inputs_op[j] for j in inputs_op],
                [activity[k] for k in range(len(activity))],
            )
            ...
            output = self._build_outputs(activity)
            self.model = tf2.keras.models.Model(
                [inputs_op[j] for j in inputs_op], output
            )
            self.model.compile(
                # TODO
                # loss='binary_crossentropy',
