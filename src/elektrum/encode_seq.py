...
def featurize_alignment(
    alignments, ltidx, build_indel=True, include_ref=False, maxlen=25, verbose=False
):
    """
    Alignment featurization function. Matrix will have the shape of (num_alignments, maxlen, 13) if build_indel is True,
    or (num_alignments, maxlen, 8) if build_indel is False
    First 4 columns indicate the reference nucleotide. If changes occur, columns 4-8 corresponding to alternative bases will be indicated. Final, columns will fill for deletions or insertion.
    """
    mats = []
    for j, aln in enumerate(alignments):
        ...
        assert len(aln[0]) <= maxlen, "alignment {} larger than maxlen: {}".format(
            j, aln
        )
        if build_indel is False and ("-" in aln[0] or "-" in aln[1]):
            ...
            for i in range(len(aln[0])):
                ...
                else:  # Mark differing bases in row with tuple indexing
                    fea[p, ltidx[k]] = 1
                    p += 1
                if verbose:
                    print(k, p, "index:", ltidx[k])
            mats.append(fea)
    mats = np.array(mats)
...
class Encoder_Electrum(Encoder_CRISPR_Net):
    ...
    def featurize_alignment(self, aln, verbose=False):
        ...
        assert len(aln[0]) <= self.tlen, "alignment {} larger than maxlen: {}".format(
            j, aln
        )
