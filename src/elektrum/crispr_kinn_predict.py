...
manager_kwargs = {
    ...
    "n_feats": 25,  # remember to change this!! (ARL: Why? What does this do?)
    "n_channels": 9,
    "batch_size": 128,
    "epochs": 30,
    "earlystop": 10,
...
def plot_dataframe(data):
    ...
    ax.set_title(
        "Positive Set Predictions (OffTargets)\nPearson=%.3f, p=%.3f, n=%i"
        % (pcc[0], pcc[1], plot_df.shape[0])
    )
    # ax = axs[1,1]
...
def predict_on_dataframe(data, is_aligned=True):
    ...
    if is_aligned:
        alignments = [
            x[1].str[::-1].tolist()
            for x in tqdm(data.iloc[:, [0, 1]].iterrows(), total=data.shape[0])
        ]
    else:
    ...
    kinn_1 = reload_from_dir(
        workdir=config["kinn_1"],
        sess=sess,
        manager_kwargs=manager_kwargs,
        model_fn=evo_params["model_fn"],
    )
    kinn_2 = reload_from_dir(
        workdir=config["kinn_2"],
        sess=sess,
        manager_kwargs=manager_kwargs,
        model_fn=evo_params["model_fn"],
    )
    # load cnn
    wd = config["dcnn_1"]
    train_hist = pd.read_table(
        os.path.join(wd, "train_history.csv"), sep=",", header=None
    )
    best_trial_id = train_hist.sort_values(2, ascending=False).head(1)[0]
    dcnn_1 = tf.keras.models.load_model(
        os.path.join(wd, "weights", "trial_%i" % best_trial_id, "bestmodel.h5")
    )
    ...
    best_trial_id = train_hist.sort_values(2, ascending=False).head(1)[0]
    dcnn_2 = tf.keras.models.load_model(
        os.path.join(wd, "weights", "trial_%i" % best_trial_id, "bestmodel.h5")
    )
