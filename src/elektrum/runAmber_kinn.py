...
def parse():
    ...
    parser.add_argument(
        "--target",
        type=str,
        choices="""wtCas9_cleave_rate_log
...
Cas9_enh_ndABA
Cas9_hypa_ndABA
Cas9_HF1_ndABA""".split(),
        required=True,
    )
    parser.add_argument(
        "--ms", type=str, choices=["finkelstein", "uniform"], required=True
    )
...
def main():
    ...
    if args.ms == "finkelstein":
        kinn_model_space = get_cas9_finkelstein_ms_with_hidden(
            use_sink_state=args.use_sink_state
        )
    else:
        kinn_model_space = get_cas9_uniform_ms(
            n_states=args.n_states,
            st_win_size=args.win_size,
            use_sink_state=args.use_sink_state,
        )
    ...
    (x_train, y_train), (x_test, y_test) = get_data(
        target=args.target, make_switch=make_switch, logbase=logbase, include_ref=False
    )

    if args.use_sink_state:
        output_op = lambda: tf.keras.layers.Lambda(
            lambda x: tf.math.log(
                tf.clip_by_value(tf.reshape(-x[:, 1], (-1, 1)), 10**-5, 10**-1)
            )
            / np.log(logbase),
            name="output_slice",
        )
        ...
        output_op = lambda: tf.keras.layers.Lambda(
            lambda x: tf.math.log(tf.clip_by_value(x, 10**-5, 10**-1))
            / np.log(logbase),
            name="output_log",
        )
    ...
    evo_params = dict(
        ...
        if args.use_sink_state
        else KineticNeuralNetworkBuilder,
